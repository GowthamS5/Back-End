...
class EmployeeController {
 ...
 static addEmployee(req, res) {
  ...
  EmployeeModel.createEmployee(
    { first_name, last_name, department, salary, DOB: formattedDOB, email, password, role, imageFilePath },
    (err, result) => {
      if (err) {
        console.error(err);
...
if (error) {
  ...
  EmployeeModel.updateEmployee(employeeId, { first_name, last_name, email, password, role, department, salary, image, userRole }, (err, result) => {
    if (err) {
      console.error(err);
